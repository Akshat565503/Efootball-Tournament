  ...
  <div>
    <NuxtLayout name="auth">
      <div class="glass-card-static p-8">
        <h2 class="font-heading font-bold text-2xl text-white text-center mb-6">Welcome back</h2>
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="password" type="password" required placeholder="••••••••"
                   class="input-glass" autocomplete="current-password" />
          </div>
        ...
        <p class="text-center text-gray-500 text-sm mt-6">
          ...
          <NuxtLink to="/auth/signup" class="text-neon-green hover:underline font-medium">Sign up</NuxtLink>
        </p>
      </div>
    </NuxtLayout>
  </div>
...
async function handleLogin() {
  ...
  try {
    ...
    if (rawMsg.includes('Invalid login credentials')) {
      error.value = 'Incorrect email or password. Please verify your credentials and try again.'
    } else if (rawMsg.includes('Email not confirmed')) {
      error.value = 'Your email address has not been verified yet. Please check your inbox for the activation link.'
    } else {
      error.value = rawMsg || 'Failed to sign in. Please try again.'
    }
  } finally {
    loading.value = false
  }
...
useHead({ title: 'Sign In — eFootball Arena' })
</script>
